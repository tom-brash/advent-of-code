...
def main():
    ...
    for i, a in enumerate(input_data.split('\n')):
        for j, b in enumerate(input_data.split('\n')):
            if i != j:
                t = s_add(a, b)
                t = [int(c) if c.isnumeric() else c for c in t]
                t = reduce(t)
                t = ''.join([str(v) for v in t])
                t_fish = parse(t)
                t_mag = t_fish.magnitude()
                if t_mag > best:
                    print('Improvement found: ', t_mag)
                    best = t_mag

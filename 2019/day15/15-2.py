'''
DAY 15-2: (Intcode) Filling in a maze

Given the implementation choice of 15-1, this code is near identical, but with a different
termination condition on propogating oxygen (all cells filled instead of reactor found).

The method to map the maze is still done randomly using a good seed.
''' 
...
def main():
    ...
    oxygen_start_location = find_cell(grid, 2)
    ...
    window.update()
    while any(1 in sublist for sublist in grid) == True:
        minutes += 1
        ...
        window.update()
        time.sleep(0.1)        
    print('Time to fill entire maze with oxygen:',minutes)
    window.mainloop()

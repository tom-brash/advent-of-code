...
def main():
    ...
    print(count_bags('shiny gold', bag_info, dict())[0])

def count_bags(bag, bag_info, memo_dict):
    if bag_info[bag] == 'none':
        return 0, memo_dict
    if bag in memo_dict:
        return memo_dict[bag], memo_dict
    total = 0
    for key, val in bag_info[bag].items():
        total += val
        n, memo_dict = count_bags(key, bag_info, memo_dict)
        total += val * n
    memo_dict[bag] = total
    return total, memo_dict

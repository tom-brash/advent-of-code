...
def main():
    ...
    unscrambler = Unscrambler('fbgdceah', instructions)
    unscrambler.setup()
    unscrambler.run()
    print(''.join(unscrambler.password))
...
class Unscrambler:
    ...
    def rel_rotate(self, a):
        for i in [1, 2, 3, 4, 6, 7, 8, 9]:
            try_pass = self.rot_left_check_f(self.password, i)
            if self.rel_rotate_check_f(try_pass, a) == self.password:
                self.password = try_pass
                break

    def rot_left_check_f(self, password, n):
        dq = deque(password)
        dq.rotate(-n)
        return list(dq)

    def rel_rotate_check_f(self, password, a):
        i = password.index(a)
        if i >= 4:
            i += 1
        dq = deque(password)
